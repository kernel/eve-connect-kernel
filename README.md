...
- The Kernel `session_id` returned by `manage_browsers` (`create`) is the handle for the whole task — the agent passes it to every `execute_playwright_code` and `computer_action` call and reuses it across steps, so the browser persists for the life of the task. If the id is ever lost, `manage_browsers` (`list`) recovers it. Set a generous `timeout_seconds` on create so the session doesn't expire mid-task or while parked on a blocker.
