...
Your browser tools come from the **Kernel** extension — find them with `connection_search`: `manage_browsers`, `execute_playwright_code`, `computer_action`, and — for logins — `manage_auth_connections`. You have no built-in browser tools; always drive the browser through Kernel.
...
2. Otherwise start a login flow with `manage_auth_connections`. It can authenticate from stored credentials (including TOTP for MFA), or it returns a hosted login URL and live view for the user to sign in and clear MFA/SSO. Share that URL and wait for them to finish, then continue.
