...
* [`extensions/kernel.ts`](https://github.com/kernel/eve-extension) mounts `@onkernel/eve-extension`, which gives the agent Kernel's browser toolset — sessions, Playwright, computer use, session replays, profiles, and managed auth — authenticated per-user through [Vercel Connect](https://vercel.com/connect), so there's no API key in the app.
